...
class hasMany extends Relation
{
    ...
    public function input()
    {
        ...
        $this->createURL = $this->createURL($this->model['route']).'?'.http_build_query([$this->relation->getForeignKey() => $id, '_standalone' => 'true']);
...
<!--			</div>-->
    ...
    {
        $edit_url = $this->editURL($this->model['route'], $item['id']).'?'.http_build_query(['_standalone' => 'true']); ?>
			...
			<span class="right">
				<span class="btn-group">
					...
					<a data-method="delete" data-token="<?= csrf_token() ?>"
					   href="<?= route('admin.model.destroy', ['slug' => $this->model['route'], 'id' => $item['id']], $absolute = true) ?>"
    ...
    public function show($values = null)
    {
        ...
        if (isset($values) && ! $values->isEmpty()) {
            ...
            foreach ($values as $value) {
                echo '<a href="'.$this->editURL($model['route'], $value->id).'" target="_blank">'.$value->$show.'</a><br>';
            }
        } else {
            echo '<em>(none)</em>';
        }
